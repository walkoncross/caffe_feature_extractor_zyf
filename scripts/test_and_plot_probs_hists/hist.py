...
def plot_multi_hists(prob_stats_list, bins=None,
...
#    for arr in prob_stats_list:
#        plt.hist(arr, bins=bins, hold=True)
##        handle_list.append(handle)
#        if label_list:
#            plt.legend(label_list[i])
    if label_list:
        plt.hist(prob_stats_list, bins=bins, hold=True, label=label_list)
        plt.legend()
    else:
        plt.hist(prob_stats_list, bins=bins, hold=True)
